...
export default function SubscriptionsPage() {
  ...
  console.log("🚀 ~ SubscriptionsPage ~ plans:", plans);
  ...
  const [cancelDialog, setCancelDialog] = useState<{
    ...
    id: string | null;
    cancelling: boolean;
  }>({ open: false, id: null, cancelling: false });
  ...
  const [changePlanDialog, setChangePlanDialog] = useState<{
    ...
    newPlanId: string;
    saving: boolean;
  }>({ open: false, subId: null, newPlanId: "", saving: false });
  ...
  // Restore plans when the create modal closes
  useEffect(() => {
    if (!createOpen) {
      fetchPlans(selectedUser);
    }
  ...
  const fetchSubscriptions = async (uid: string) => {
    ...
    try {
      ...
      const result = (data as Record<string, unknown>)
        ?.listUserSubscriptions as SubscriptionResponse;
  ...
  const handleCreate = async () => {
    if (!newSub.planId || !newSub.userId) {
      toast.error("El plan y el usuario son obligatorios");
      return;
    }
    setCreating(true);
    try {
      const { data } = await apolloClient.mutate({
        ...
        variables: {
          subscription: { planId: newSub.planId, userId: newSub.userId },
        },
      });
      const result = (data as Record<string, unknown>)
        ?.createSubscription as SubscriptionResponse;
      if (result?.success) {
        toast.success("Suscripción creada");
        setCreateOpen(false);
        setUserId(newSub.userId);
        fetchSubscriptions(newSub.userId);
      } else {
  ...
  const handleCancel = async () => {
    ...
    try {
      const { data } = await apolloClient.mutate({
        ...
        variables: { input: { subscriptionId: cancelDialog.id } },
      });
      const result = (data as Record<string, unknown>)
        ?.cancelSubscription as SubscriptionResponse;
      if (result?.success) {
        toast.success("Suscripción cancelada");
        setCancelDialog({ open: false, id: null, cancelling: false });
      ...
      setCancelDialog(p => ({ ...p, cancelling: false }));
    }
  };
  ...
  const handleChangePlan = async () => {
    ...
    try {
      await apolloClient.mutate({
        ...
        variables: {
          subscriptionId: changePlanDialog.subId,
          newPlanId: changePlanDialog.newPlanId,
        },
      });
      ...
      setChangePlanDialog(p => ({ ...p, saving: false }));
    }
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Gestiona las suscripciones de los usuarios"
        actions={
      ...
      {loading ? (
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-8 text-center text-muted-foreground">
            {userId
              ? "No se encontraron suscripciones para este usuario."
              : "Selecciona un usuario para ver sus suscripciones."}
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span>
                        Fin: {new Date(sub.endDate).toLocaleDateString("es-ES")}
                      </span>
                    </div>
                  </div>
                  <DropdownMenu>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={newSub.planId}
                onValueChange={v => setNewSub({ ...newSub, planId: v })}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {plans.map(p => (
                    <SelectItem key={p.id} value={p.id}>
                      {p.name} — {p.amount} {p.currency.toUpperCase()}/
                      {p.interval}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      <Dialog
        ...
        onOpenChange={o => setChangePlanDialog(p => ({ ...p, open: o }))}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select
                value={changePlanDialog.newPlanId}
                onValueChange={v =>
                  setChangePlanDialog(p => ({ ...p, newPlanId: v }))
                }
              >
                <SelectTrigger>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setChangePlanDialog(p => ({ ...p, open: false }))}
            >
              Cancelar
            </Button>
            <Button
              onClick={handleChangePlan}
              disabled={changePlanDialog.saving}
            >
              {changePlanDialog.saving ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : null}{" "}
              Cambiar
            </Button>
      ...
      <ConfirmDialog
        ...
        onOpenChange={o => setCancelDialog(p => ({ ...p, open: o }))}
        title="Cancelar suscripción"
        description="¿Estás seguro de que quieres cancelar esta suscripción?"
        confirmLabel="Cancelar suscripción"
        onConfirm={handleCancel}
        variant="destructive"
        loading={cancelDialog.cancelling}
      />
    </div>
  );
}
