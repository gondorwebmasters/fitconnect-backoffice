...
export function CustomBreadcrumbs({
  ...
  const renderHeading = (
    <Typography variant="h4" sx={[{ mb: 2 }, ...(Array.isArray(slotProps?.heading) ? slotProps.heading : [slotProps?.heading])]}>
      {heading}
    </Typography>
  );
  ...
  const renderAction = (
    <Box sx={[{ flexShrink: 0 }, ...(Array.isArray(slotProps?.action) ? slotProps.action : [slotProps?.action])]}>
      {action}
    </Box>
  );
